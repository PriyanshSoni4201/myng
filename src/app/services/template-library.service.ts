...
export class TemplateLibraryService {
  ...
  public getSnippet(item: any): string {
    // This router now cleanly directs each component type to its own dedicated function.
    ...
    if (item.isTableViewWithLine && item.column === 0) {
      return this.buildStandardTable(item);
    }
    if (item.isTitleValueComponent && item.column === 3) {
  ...
  /**
   * TEMPLATE 1: Patient Info Card
   * For: isHeaderComponent: true
   */
  ...
  /**
   * TEMPLATE 3: Standard Table (Natural Fit)
   * For: isTableViewWithLine: true, column: 0
   */
    ...
    const rowsHtml = tableData
      .map((row: any) => {
        const cellsHtml = headers
          ...
          .join('');
        return `<tr style="border-bottom: 1px solid #eee;">${cellsHtml}</tr>`;
      })
      .join('');
  ...
  /**
   * TEMPLATE 4: Key-Value List (3-Column Grid)
   * For: isTitleValueComponent: true, column: 3
   */ private buildKeyValueList(item: any): string {
    ...
    const recordsHtml = tableData
      .map((record: any, index: number) => {
        // Separate the "No." field from the rest of the data
        const { 'No.': noValue, ...restData } = record;
        const entries = Object.entries(restData);

        // Create the HTML for the first column, which only contains "No."
        const firstColumnHtml = `
        <div>
          <div style="color: #555; font-size: 10px;">No.</div>
          <div style="font-weight: bold; margin-top: 2px;">${noValue}</div>
        </div>
      `;

        // Create the HTML for the remaining items, which will be placed in the 3-column grid
        const otherColumnsHtml = entries
          .map(
            ([key, value]) => `
        <div>
          <div style="color: #555; font-size: 10px;">${key}</div>
          <div style="font-weight: bold; margin-top: 2px;">${value}</div>
        </div>
      `
          )
          .join('');
        ...
        // The main layout is now a flex container for the two main sections
        return `
        <div style="display: flex; gap: 15px; ${separatorStyle}">
          <!-- Column 1: Just for "No." -->
          <div style="flex: 0 0 50px;">
            ${firstColumnHtml}
          </div>
          <!-- Columns 2, 3, 4: A 3-column grid for the rest of the data -->
          <div style="flex: 1; display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px 15px;">
            ${otherColumnsHtml}
          </div>
        </div>
      `;
    ...
    return `
      <div style="margin-top: 15px; font-family: sans-serif; font-size: 11px;">
        <div style="font-weight: bold; font-size: 13px; padding-bottom: 2px; margin-bottom: 8px; ${borderStyle}">
          ${headerTitle}
        </div>
        <div>
          ${recordsHtml}
  ...
  /*
  SAMPLE INPUT for buildKeyValueList:
  {
    "header": "Hospitalization & Surgical History",
    "isTitleValueComponent": true,
    "column": 3,
    "tableData": [ { ... record 1 ... }, { ... record 2 ... } ]
  }
  */
}
