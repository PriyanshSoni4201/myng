...
export class TemplateLibraryService {
  ...
  public getSnippet(item: any): string {
    // This router now uses the clean 'contentType' property.
    switch (item.contentType) {
      ...
      default:
        // The fallback message now correctly identifies the missing content type.
        return `<div style="color: #ccc; border: 1px dashed #ccc; padding: 10px; margin-top: 15px;">(Component type '${item.contentType || 'undefined'}' not yet implemented)</div>`;
    }
  }
  ...
  private buildPatientInfoCard(item: any): string {
    ...
    const leftColumnData = {'UHID': data.UHID, 'Encounter No.': data['Encounter No.'], 'Address': data.Address, 'Diagnosis': data.Diagnosis};
    const rightColumnData = {'Consultant': data['Ref. By'], 'Ref. By': data['Ref. By'], 'Payer Class': data['Payer Class'] || 'Self'};
    const createColumnHtml = (columnData: Record<string, string>): string => { return Object.entries(columnData).map(([key, value]) => `<tr><td style="padding: 2px 8px 2px 2px; font-weight: bold; vertical-align: top; white-space: nowrap;">${key}</td><td style="padding: 2px; vertical-align: top;">${value || ''}</td></tr>`).join(''); };
  ...
  private buildSimpleParagraph(item: any): string {
    ...
    return `<div style="margin-top: 15px; font-family: sans-serif; font-size: 11px;"><div style="font-weight: bold; font-size: 13px; padding-bottom: 2px; margin-bottom: 5px; border-bottom: 1px solid #c0c0c0;">${headerTitle}</div><p style="line-height: 1.4; padding-top: 2px; margin: 0;">${content}</p></div>`;
  }
  ...
  private buildStandardTable(item: any): string {
    ...
    const headerHtml = headers.map(h => `<th style="text-align: left; padding: 4px; font-weight: bold;">${h}</th>`).join('');
    const rowsHtml = tableData.map((row: any) => {
      const cellsHtml = headers.map(h => `<td style="padding: 4px; vertical-align: top;">${row[h]}</td>`).join('');
      return `<tr>${cellsHtml}</tr>`;
    }).join('');
    return `<div style="margin-top: 15px; font-family: sans-serif; font-size: 11px;"><div style="font-weight: bold; font-size: 13px; padding-bottom: 2px; margin-bottom: 5px; border-bottom: 1px solid #c0c0c0;">${headerTitle}</div><table style="width: 100%; border-collapse: collapse; margin-top: 5px;"><thead><tr style="border-bottom: 1px solid #999;">${headerHtml}</tr></thead><tbody>${rowsHtml}</tbody></table></div>`;
  }

  private buildKeyValueList(item: any): string {
    const headerTitle = item.header || '';
    const tableData = item.tableData || [];
    const recordsHtml = tableData.map((record: any, index: number) => {
      ...
      const otherColumnsHtml = entries.map(([key, value]) => `<div><div style="color: #555; font-size: 10px;">${key}</div><div style="font-weight: bold; margin-top: 2px;">${value}</div></div>`).join('');
      const separatorStyle = index > 0 ? 'border-top: 1px solid #eee; padding-top: 12px; margin-top: 12px;' : '';
    ...
    return `<div style="margin-top: 15px; font-family: sans-serif; font-size: 11px;"><div style="font-weight: bold; font-size: 13px; padding-bottom: 2px; margin-bottom: 8px; border-bottom: 1px solid #c0c0c0;">${headerTitle}</div><div>${recordsHtml}</div></div>`;
  }
  ...
  private buildSignatureBlock(item: any): string {
    ...
    const doctorsHtml = doctors.map((doctor: string) => 
      `<div style="display: inline-block; text-align: center; margin-left: 50px;">
        <div style="height: 40px; border-bottom: 1px solid #333;"></div>
        <div style="font-weight: bold;">${doctor}</div>
      </div>`
    ).join('');

    return `
      <div style="margin-top: 40px; font-family: sans-serif; font-size: 11px; text-align: right;">
        ${doctorsHtml}
